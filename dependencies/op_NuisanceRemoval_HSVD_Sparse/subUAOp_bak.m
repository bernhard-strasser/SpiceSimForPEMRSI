...
% 		ktHr                  = fft(fft(ImtHr,[],1),[],2)/sqrt(NyHr)/sqrt(NxHr);
	...
	function y                = AcOp(x)
        ...
        % Non-Uniform (Slow) Fourier Transform
        ktHr = ktHr .* params.Operators.DCFPreG;
        ktHr = reshape(ktHr, [prod(params.Operators.InDataSize(1:2)) numel(ktHr)/prod(params.Operators.InDataSize(1:2))]);
        ImtHr = params.Operators.sft2_Oper' * ktHr * size(params.Operators.sft2_Oper,2);
		ImtHr                 = reshape(ImtHr,[params.Operators.OutDataSize(1:3) Nt]);
